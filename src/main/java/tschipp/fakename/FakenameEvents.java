...
@EventBusSubscriber(modid = FakeName.MODID)
    ...
    public static void onJoinWorld(PlayerEvent.PlayerLoggedInEvent event) {
        ...
        if (!player.level().isClientSide()) {
            if (player.getPersistentData().contains("fakename")) {
                FakeName.sendPacket(player,
                        player.getPersistentData().getString("fakename").orElse(""), 0);
            }
            ...
            for (Player other : player.level().getServer().getPlayerList().getPlayers()) {
                if (other.getPersistentData().contains("fakename")) {
                    PacketDistributor.sendToPlayer((ServerPlayer) player,
                            new FakeNamePayload(
                                    other.getPersistentData().getString("fakename").orElse(""),
                                    other.getId(), 0));
                }
            }
        }
        ...
        if (event.getTarget() instanceof Player targetPlayer) {
            if (targetPlayer.getPersistentData() != null &&
                ...
                PacketDistributor.sendToPlayer(toReceive,
                        new FakeNamePayload(
                                targetPlayer.getPersistentData().getString("fakename").orElse(""),
